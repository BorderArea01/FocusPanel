...
#### 1. 🗂️ 桌面图标与文件收纳 (Desktop Organizer)
- [x] **核心逻辑**: 实现桌面文件扫描与监视 (File Watcher)。
- [x] **智能分类**: 开发基于文件类型/扩展名的自动分类算法。
- [x] **自定义分区**: 实现可拖拽、可命名的桌面分区 UI (支持手动创建分区、右键移动文件)。
- [x] **视图切换**: 实现“按类别/分区”和“按时间线”的一键切换。
- [x] **UI 增强**: 实现更美观的卡片式布局和交互动画。
- [x] **开机自启**: 添加注册表或启动文件夹集成。
